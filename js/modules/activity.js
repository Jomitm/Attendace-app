...
export class ActivityMonitor {
    constructor() {
        ...
        if (AppDB) this.initCommandListener();
    }

    initCommandListener() {
        if (this.commandListener) return; // Already running

        if (AppDB && AppDB.listen) {
            console.log("Activity Monitor: Initializing System Command Listener...");
            this.commandListener = AppDB.listen('system_commands', (commands) => {
    ...
    async performSilentAudit(slot) {
        ...
        // Save to DB
        try {
            await AppDB.add('location_audits', logData);
...
export const AppActivity = new ActivityMonitor();
if (typeof window !== 'undefined') window.AppActivity = AppActivity;
