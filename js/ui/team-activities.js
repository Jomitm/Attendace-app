        ...
        <tr>
            ...
            <td>
                <div class="team-activities-row-actions">
                    ...
                    ` : ''}
                    ${row.type === 'work' ? `
                        <button class="team-activities-row-btn secondary" data-action="add-to-outlook" data-task-title="${safeHtml(row.task)}" data-task-date="${safeHtml(row.date)}" title="Add to Outlook">
                            <i class="fa-regular fa-calendar-plus"></i> Outlook
                        </button>
                    ` : ''}
                </div>
            </td>
...
function bindEvents() {
    ...
    document.addEventListener('click', async (event) => {
        ...
        if (actionBtn) {
            ...
            if (action === 'add-to-outlook') {
                const taskTitle = actionBtn.getAttribute('data-task-title') || 'Task';
                const taskDate = actionBtn.getAttribute('data-task-date') || new Date().toISOString().slice(0, 10);
                import('../utils/ical.js').then(({ buildICS, downloadICS }) => {
                    ...
                    downloadICS(`task-${taskDate}.ics`, ics);
                });
            }
        }
