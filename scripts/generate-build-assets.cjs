...
const { ensureBuildMetaFile } = require('./build-meta.cjs');

const cwd = process.cwd();
const publicDir = path.join(cwd, 'public');
const meta = ensureBuildMetaFile(cwd);
...
function createServiceWorker(buildMeta) {
    const buildId = JSON.stringify(String(buildMeta.buildId || 'local'));
...
writeJson(path.join(publicDir, 'version.json'), meta);
fs.writeFileSync(path.join(publicDir, 'sw.js'), createServiceWorker(meta));
